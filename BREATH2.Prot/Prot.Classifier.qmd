...
lasso_cv_outer_fold <- function(test_fold) {
  ...
  #foldid <- sample(rep(1:nfolds, length.out = sum(train))) #uncomment to run 5b.1
  

  
  
#------------------------Select all proteins path (5b.1)-----------------------#  
...
#--------------------Select best lambda path (5b.2)----------------------------#  
  
  
 # Fit full lambda path (no CV)
fit <- glmnet::glmnet(
  X[train, ], y[train],
  family = "binomial",
  standardize = FALSE
...
if (any(nz == 3)) {
  # Choose the *largest* lambda with exactly 3 (strongest regularization)
  lambda_k3 <- fit$lambda[which(nz == 3)[1]]
} else {
  # Fail-safe: choose lambda with closest >3, then trim to top3
...
nonzero <- data.frame(
  ...
  list(pred = preds, mod = fit, coef = nonzero) #Change to mod = mod to run 5b.1
}




#------------------------Run LASSO on all folds (5b.3)-------------------------#
...
Mean AUC: 0.904, 95% CI[0.846, 0.963]
